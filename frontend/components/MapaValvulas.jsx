...
// El nombre/estado de una válvula vienen del backend y pueden haber sido
// escritos por cualquier operario: nunca se insertan sin escapar en el popup.
function esc(s) {
    ...
    (async () => {
      ...
      geojson.features.forEach((f) => {
        ...
        L.marker([lat, lng], { icon: valveIcon })
          .addTo(map)
          .bindPopup(
            `<b>${esc(p.nombre ?? 'Válvula')}</b><br/>DN ${esc(p.diametro ?? '—')} · ${esc(p.estado ?? '—')}<br/>` +
